...
function frameLabel(p: number | null): string {
  if (p === null) return 'NO PREDICTION'
  if (p >= 0.6) return 'FAKE'
  if (p <= 0.4) return 'REAL'
  return 'UNCERTAIN'
}

function buildFrameLines(result: AnalysisResult): string[] {
  ...
  if (frameRows.length === 0) return ['No per-frame breakdown was returned for this analysis.']
  return frameRows.map((r) => {
    const faces = r.error ? 'n/a' : String(r.faces)
    const prob = r.prediction === null ? '—' : formatPercent(r.prediction, 1)
    return `Frame #${r.index + 1} | Faces: ${faces} | Fake probability: ${prob} | ${frameLabel(r.prediction)}`
  })
}
...
export function generateVerdictReportPdf(result: AnalysisResult): Blob {
  ...
  const sections: ReportSection[] = [
    ...
    },
    {
      ...
      rows: [
        { label: 'Frames Sampled', value: String(result.frames_sampled) },
        { label: 'Frames with Faces', value: String(result.frames_with_faces) },
        { label: 'Faces Detected', value: String(result.faces_detected ?? result.frames_with_faces ?? 0) },
        { label: 'Fake Probability (mean)', value: formatPercent(result.fake_probability) },
        { label: 'Real Probability', value: formatPercent(result.real_probability) },
        { label: 'Mean Probability', value: formatPercent(result.mean_probability) },
        { label: 'Median Probability', value: formatPercent(result.median_probability) },
        { label: 'Consistency (StdDev)', value: formatPercent(result.std_probability) },
      ],
    },
    { title: 'Frame Analysis', paragraphs: buildFrameLines(result) },
    buildRppgSection(result),
    { title: 'Explanation', paragraphs: [explanation] },
    {
      title: 'Model',
      rows: [
        { label: 'Architecture', value: result.model_name || 'EfficientNet-B4' },
        { label: 'Checkpoint', value: result.model_version || 'best_model.pt' },
  ...
  return buildReportPdf({
    ...
    verdict,
    sections,
    footer: 'Generated by BioVision - EfficientNet-B4 + CHROM rPPG late-fusion analysis',
  })
}
